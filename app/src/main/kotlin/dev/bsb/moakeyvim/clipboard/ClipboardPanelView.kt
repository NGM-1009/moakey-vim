...
class ClipboardPanelView @JvmOverloads constructor(
    ...
    init {
        ...
        emptyText = TextView(context).apply {
            text = context.getString(dev.bsb.moakeyvim.R.string.clipboard_empty_hint)
    ...
    private fun buildHeader(dp4: Int, dp8: Int): LinearLayout {
        ...
        return LinearLayout(context).apply {
            ...
            pinToggleBtn = ImageView(context).apply {
                setImageResource(dev.bsb.moakeyvim.R.drawable.ic_pin)
            ...
            addView(ImageView(context).apply {
                setImageResource(dev.bsb.moakeyvim.R.drawable.ic_trash)
                scaleType = ImageView.ScaleType.CENTER_INSIDE
                setPadding(dp8, dp4, dp8, dp4)
                isClickable = true
                isFocusable = true
                ...
                setOnClickListener {
                    showConfirmMenu(
                        message = context.getString(dev.bsb.moakeyvim.R.string.clipboard_clear_confirm),
                        onConfirm = {
    ...
    private fun showConfirmMenu(message: String, onConfirm: () -> Unit) {
        ...
        menuCard.addView(LinearLayout(context).apply {
            ...
            addView(TextView(context).apply {
                text = context.getString(dev.bsb.moakeyvim.R.string.clipboard_clear_cancel)
                textSize = 13f
            ...
            addView(TextView(context).apply {
                text = context.getString(dev.bsb.moakeyvim.R.string.clipboard_clear_yes)
                textSize = 13f
                setPadding(dp12, dp8, dp12, dp8)
                setTextColor(fgColor)
                isClickable = true
    ...
    private fun showTypeActions(entry: ClipboardEntry, type: EntryType) {
        showInlineMenu(buildList {
            add(context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_paste) to {
                ClipboardRepository.use(context, entry.id)
                refresh(context)
                onPaste?.invoke(entry.text)
            })
            when (type) {
                EntryType.URL -> add(context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_open_url) to {
                    ClipboardRepository.use(context, entry.id)
                    refresh(context)
                    openUrl(entry.text)
                })
                EntryType.EMAIL -> add(context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_send_email) to {
                    ClipboardRepository.use(context, entry.id)
                    refresh(context)
    ...
    private fun showContextMenu(entry: ClipboardEntry) {
        showInlineMenu(buildList {
            add(context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_edit) to { onEdit?.invoke(entry) })
            val pinLabel = if (entry.pinned)
                context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_unpin)
            else
                context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_pin)
            add(pinLabel to {
                if (entry.pinned) ClipboardRepository.unpin(context, entry.id)
                else ClipboardRepository.pin(context, entry.id)
                refresh(context)
            })
            add(context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_add_hotstring) to { onAddHotstring?.invoke(entry) })
            add(context.getString(dev.bsb.moakeyvim.R.string.clipboard_action_delete) to {
    ...
    inner class ClipboardAdapter : RecyclerView.Adapter<ClipboardAdapter.VH>() {
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
            ...
            val themedCtx = ContextThemeWrapper(context, dev.bsb.moakeyvim.R.style.Theme_MoaKeyVim_Settings)
            val neg = (-12 * dp).toInt()
