...
class SettingsImportParseTest {
    ...
    private fun buildExportJson(): JSONObject {
        val settingsObj = JSONObject().apply {
            ...
            put(SettingsPreferences.KEY_HW_CAPSLOCK_TO_CTRL, false)
            put(SettingsPreferences.KEY_HW_TAB_VIM_MODE, true)
            // int 항목 (JSON number 타입으로 저장됨)
            put(SettingsPreferences.KEY_FLOATING_INDICATOR_X, 936)
            put(SettingsPreferences.KEY_FLOATING_INDICATOR_Y, 1398)
            // string 항목
    ...
    fun `settings 섹션 파싱 - boolean과 string 타입 구분`() {
        ...
        settingsObj!!.keys().forEach { key ->
            ...
            when {
                key in booleanKeys -> settingsEdits.add(key to settingsObj.optBoolean(key))
                key in intKeys -> settingsEdits.add(key to settingsObj.optInt(key))
                else -> {
    ...
    fun `settings 값 정확성 검증`() {
        ...
        settingsObj.keys().forEach { key ->
            if (key !in allowedKeys) return@forEach
            when {
                key in booleanKeys -> edits[key] = settingsObj.optBoolean(key)
                key in intKeys -> edits[key] = settingsObj.optInt(key)
                else -> {
                    val value = settingsObj.optString(key)
        ...
        assertEquals(936, edits[SettingsPreferences.KEY_FLOATING_INDICATOR_X])
        assertEquals(1398, edits[SettingsPreferences.KEY_FLOATING_INDICATOR_Y])
        assertEquals(false, edits[SettingsPreferences.KEY_HW_CAPSLOCK_TO_CTRL])
        assertEquals(true, edits[SettingsPreferences.KEY_HW_TAB_VIM_MODE])
    }

    @Test
    fun `floating_indicator 좌표는 Int 타입으로 파싱되어야 함`() {
        val root = buildExportJson()
        val settingsObj = root.optJSONObject("settings")!!
        ...
        val x = edits[SettingsPreferences.KEY_FLOATING_INDICATOR_X]
        val y = edits[SettingsPreferences.KEY_FLOATING_INDICATOR_Y]
        assertTrue("floating_indicator_x는 Int여야 함, 실제: ${x?.javaClass}", x is Int)
        assertTrue("floating_indicator_y는 Int여야 함, 실제: ${y?.javaClass}", y is Int)
        assertEquals(936, x)
        assertEquals(1398, y)
    }
